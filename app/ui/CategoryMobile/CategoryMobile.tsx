...
export default function CategoryMobile({ menus }: any) {
  if (!menus || !Array.isArray(menus)) return null; // <-- Prevent render if menus is undefined or not an array

  return (
    <div
      id="main_category_show_home"
      className="flex md:hidden w-full flex-row overflow-x-scroll scrollbar-hide gap-x-2 pb-0 pt-[10px] px-4"
    >
      {menus.map((item: any, index: any) => (
        <Link key={index} href={`/category/${item?.slug}`}>
          <div className="flex grow items-center justify-center flex-col gap-y-1">
